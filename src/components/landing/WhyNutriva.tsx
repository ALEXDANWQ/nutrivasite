...
const items = [
  { icon: IconBolt, title: "Быстро", text: "Добавление продуктов занимает минимум времени.", n: "01" },
  { icon: IconEye, title: "Понятно", text: "Все главные показатели видны сразу.", n: "02" },
  { icon: IconLeaf, title: "Спокойно", text: "Интерфейс не давит, а помогает сохранять ритм.", n: "03" },
];

export function WhyNutriva() {
  return (
    <section className="relative overflow-hidden py-16 sm:py-24 md:py-32">
      <div aria-hidden className="absolute inset-0 -z-10 bg-gradient-to-b from-transparent via-[#EEF8F1]/50 to-transparent" />
          ...
          {items.map((it, i) => {
            ...
            return (
              <Reveal key={it.title} delay={i * 0.1}>
                <div className="group relative h-full overflow-hidden rounded-2xl border border-white/70 bg-white/80 p-5 text-left backdrop-blur-md transition-all duration-[700ms] ease-[cubic-bezier(.22,1,.36,1)] hover:-translate-y-1.5 hover:shadow-elevated sm:rounded-3xl sm:p-8"
                  style={{ boxShadow: "0 1px 0 rgba(255,255,255,0.8) inset, 0 12px 40px -16px rgba(17,24,39,0.10)" }}
                >
                  ...
                  <h3 className="text-[19px] sm:text-[22px] font-bold tracking-tight">{it.title}</h3>
                  <p className="mt-2 sm:mt-2.5 text-[13.5px] sm:text-[14px] text-muted-foreground leading-relaxed">{it.text}</p>
                </div>
              </Reveal>
            );
          })}
